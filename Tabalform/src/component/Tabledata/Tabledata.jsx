  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
    {users.length > 0 ? (
      users.map((userdata) => (
        <div
          key={userdata._id}
          className="bg-white shadow-md rounded-lg p-5 transition-transform transform hover:scale-105 hover:shadow-2xl hover:bg-gradient-to-tr from-pink-200 to-purple-50"
        >
          <div className="flex flex-col items-center">
            <img
              className="w-28 h-28 rounded-full border-4 border-purple-500 mb-4 transition-transform transform hover:scale-110"
              src={img}
              alt={userdata.name}
            />
            ...
            <div className="flex space-x-3">
              <button
                className="px-4 py-1 bg-green-600 text-white rounded-full text-sm transition-all duration-300 transform hover:bg-green-700 hover:scale-105 hover:shadow-md"
                onClick={() => handleViewOpen(userdata)}
              >
                View
              </button>
              <button
                className="px-4 py-1 bg-blue-600 text-white rounded-full text-sm transition-all duration-300 transform hover:bg-blue-700 hover:scale-105 hover:shadow-md"
                onClick={() => handleUpdate(userdata._id)}
              >
                Update
              </button>
              <button
                className="px-4 py-1 bg-red-600 text-white rounded-full text-sm transition-all duration-300 transform hover:bg-red-700 hover:scale-105 hover:shadow-md"
